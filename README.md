...
- **Image formats** — Common and modern stills, Photoshop documents, and 60+ camera RAW formats (RAW is viewing-only).
  ...
  - **Photoshop**: PSD & PSB with full blend-mode support (including Hue, Saturation, Color, Luminosity), vector mask support, accurate rendering of Bitmap, Indexed, Duotone, Multichannel, and Lab colour modes. A RAM safety check runs before loading large PSB documents, and high-bit-depth files render with accurate tone mapping.
...
| **Display** | Full-screen toggle, scale-mode selector, OSD info visibility, tiled-rendering threshold (best left at your graphics device maximum — usually no need to change; shown so you can screenshot it when contacting support), **High-Quality RAW preview** toggle, and **Demosaicing Mode (CPU / GPU)** when HQ RAW is enabled |
