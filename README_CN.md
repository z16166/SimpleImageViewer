...
- **氛围感转场动画**：提供多种专业的双纹理过渡效果，包括 **Cross-Fade (淡入淡出)**、**Zoom & Fade (缩放淡入)**、**Slide (滑入)**、**Push (推入)** 以及 **Page Flip (翻页)**。支持 50ms 到 2000ms 的时长调节。
- **背景音乐同步**：支持播放背景音乐。
- **跨平台中文支持**：自动加载系统 CJK 字体（如微软雅黑、萍方、Noto CJK），确保中文路径和文件名完美显示。
路径中的中文、日文、韩文字符
