...
- **图片打印** — 直接从应用内打印图片。
  - **Windows**: 调用系统原生打印向导 (`ShellExecuteW`)。支持高保真 JPEG (95% 质量) 和自动 Alpha 透明度扁平化处理。
...
- **背景音乐播放** — 高保真音频回放，支持 MP3、FLAC、OGG、WAV、AAC、M4A；可选择单个文件或整个文件夹（递归搜索）
- **CUE 索引与精确导航** — 完整支持 `.cue` 文件（如 WAV+CUE, FLAC+CUE 等）。即使针对体积巨大的单体音频文件，也能基于 `INDEX 01` 时间戳实现精确的曲目跳转
- **智能元数据解析** — 自动识别歌曲名、艺术家和音轨号；支持文件内嵌标签及外部 CUE 描述，并具备 UTF-8/GBK 编码自动回退兼容
...
- **多语言支持 (i18n)** — 界面不仅可以根据操作系统自动识别并切换相应的语言（支持简体中文、繁体中文（台湾/香港）和英文），用户也可利用设置内的语言选项手动切换。
