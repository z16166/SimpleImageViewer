...
- **目录树导航** — 在嵌入式侧栏或独立窗口中浏览文件夹，列表带缩略图预览与可排序列；可快速跳转到桌面、文档、盘符与网络位置（Linux 上部分格式的列表缩略图可能比 Windows/macOS 慢，主窗口浏览不受影响）
...
| 设置项 | 说明 |
|---|---|
| **Library（图库）** | 通过 **选择** 按钮或主窗口 `Ctrl + O` 选取图片文件夹，控制递归扫描、缓存预加载、存在同名 JPG/JPEG 时跳过 RAW，以及是否恢复上次记忆的查看进度 |
