...
impl ImageViewerApp {
            ...
            && let Ok(files) = rx.try_recv()
        {
            ...
            if !files.is_empty() {
                ...
                if let Some(last_path) = &self.settings.last_music_file
                    && let Some(idx) = files.iter().position(|p| p == last_path)
                {
                    start_idx = Some(idx);
                }
    ...
    pub(crate) fn open_music_file_dialog(&mut self, frame: &eframe::Frame) {
        self.request_music_file_picker(frame);
    }

    pub(crate) fn open_music_dir_dialog(&mut self, frame: &eframe::Frame) {
        self.request_folder_picker(
            frame,
            crate::app::folder_picker::FolderPickerPurpose::MusicDirectory,
            self.settings.music_path.clone(),
        );
    }
