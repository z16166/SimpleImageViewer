...
#[allow(dead_code)]
fn encode_jpeg_to_memory<W: std::io::Write + std::io::Seek>(
    img: &RgbImage,
    writer: W,
    quality: u8,
) -> Result<(), String> {
...
fn invoke_system_print(path: &Path) -> Result<(), String> {
    ...
    let path_w: Vec<u16> = path
        ...
        .collect();
    let verb_w: Vec<u16> = std::ffi::OsStr::new("print")
        .encode_wide()
        .chain(std::iter::once(0))
        .collect();
...
fn export_to_pdf_and_print(img: &RgbImage, out_path: &Path) -> Result<(), String> {
    ...
    let quality = 90;
    let mut encoder =
        ::image::codecs::jpeg::JpegEncoder::new_with_quality(&mut compressed_buffer, quality);
    ...
    dict.insert(
        "Type".to_string(),
        ::printpdf::DictItem::Name(b"XObject".to_vec()),
    );
    dict.insert(
        "Subtype".to_string(),
        ::printpdf::DictItem::Name(b"Image".to_vec()),
    );
    dict.insert("Width".to_string(), ::printpdf::DictItem::Int(width as i64));
    dict.insert(
        "Height".to_string(),
        ::printpdf::DictItem::Int(height as i64),
    );
    dict.insert(
        "ColorSpace".to_string(),
        ::printpdf::DictItem::Name(b"DeviceRGB".to_vec()),
    );
    dict.insert("BitsPerComponent".to_string(), ::printpdf::DictItem::Int(8));
    dict.insert(
        "Filter".to_string(),
        ::printpdf::DictItem::Name(b"DCTDecode".to_vec()),
    );
