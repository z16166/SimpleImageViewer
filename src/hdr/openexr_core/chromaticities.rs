...
pub(crate) fn deep_scanline_flatten_rgba_via_imf(
    ...
    let rgba_len = (expected_w as usize)
        ...
        .ok_or_else(|| {
            format!(
                "OpenEXR deep scanline buffer size overflow: {expected_w}x{expected_h}"
            )
        })?;
...
pub(crate) fn rgba_input_scanline_flatten_rgba_via_imf(path: &Path) -> Result<Vec<f32>, String> {
    ...
    if code == -5 {
        ...
        let need = (w as usize)
            ...
            .ok_or_else(|| {
                format!(
                    "OpenEXR IMF flatten buffer size overflow: {w}x{h}"
                )
            })?;
...
pub(crate) fn extract_rgba32f_tile_from_flat_buffer(
    ...
    let row_stride = (image_width as usize)
        .checked_mul(4)
        .ok_or_else(|| {
            format!("OpenEXR tile row stride overflow: width={image_width}")
        })?;
    let out_len = (width as usize)
        ...
        .ok_or_else(|| {
            format!(
                "OpenEXR tile buffer size overflow: {width}x{height}"
            )
        })?;
