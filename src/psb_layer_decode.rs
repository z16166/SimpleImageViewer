...
pub(crate) fn decode_one_layer(
    ...
    {
        match rasterize_vector_mask(vm, record.left, record.top, width, height) {
...
fn read_path_coord(rec: &[u8; 26], offset: usize) -> f64 {
    i32::from_be_bytes([
        rec[offset],
        rec[offset + 1],
        rec[offset + 2],
...
fn read_path_point(rec: &[u8; 26], base: usize) -> (f64, f64) {
...
pub(crate) fn rasterize_vector_mask(
    ...
    for poly in &parsed.subpaths {
        ...
        for py in bbox_min_y..bbox_max_y {
            ...
            let row_start = (py as usize).wrapping_mul(w as usize);
            for px in bbox_min_x..bbox_max_x {
                ...
                if inside {
                    let idx = row_start.wrapping_add(px as usize);
                    if idx < mask.len() {
                        mask[idx] = 255;
                    }
                }
