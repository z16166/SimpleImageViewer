...
use crate::loader::{
    ...
    check_decode_cancel_str, hdr_sdr_fallback_rgba8_or_placeholder,
};
use std::path::Path;
use std::sync::Arc;
...
pub(crate) fn load_hdr(
    ...
    if is_exr_path(path) {
        return load_detected_exr(path, cancel);
    }
    let mmap = Arc::new(crate::mmap_util::map_file(path)?.0);
    load_hdr_from_mmap(path, mmap, hdr_target_capacity, hdr_tone_map, cancel)
}
...
pub(crate) fn load_hdr_from_mmap(
    ...
    if let Some(image_data) =
        try_load_disk_backed_radiance_hdr_from_mmap(path, Arc::clone(&mmap), hdr_tone_map, cancel)?
    {
        return Ok(image_data);
    }

    // Full-file Radiance float decode -- poll before the expensive scanline pass.
    check_decode_cancel_str(cancel)?;
    let hdr =
        crate::hdr::decode::decode_radiance_hdr_image_from_mmap(mmap.as_ref(), Some(path), cancel)?;
...
pub(crate) fn try_load_disk_backed_exr_hdr(
    ...
    let mmap = Arc::new(crate::mmap_util::map_file(path)?.0);
    try_load_disk_backed_exr_hdr_from_mmap(path, mmap, cancel)
}
...
pub(crate) fn try_load_disk_backed_exr_hdr_from_mmap(
    ...
    route_exr_tiled_source(path, source, Some(file_bytes), cancel)
}
...
pub(crate) fn exr_tiled_source_to_static_hdr(
    ...
    // Full-image EXR extract -- poll before chunk/tile orchestration.
    check_decode_cancel_str(cancel)?;
    let tile = source.extract_tile_rgba32f_arc_with_cancel(
        0,
        0,
        source.width(),
        source.height(),
...
pub(crate) fn load_deep_exr_from_mmap(
    ...
    check_decode_cancel_str(cancel)?;
    match crate::hdr::exr_tiled::decode_deep_exr_image_from_mmap(path, Arc::clone(&mmap), cancel) {
        Ok(hdr) => {
            let fallback = DecodedImage::from_hdr_sdr_fallback(
                hdr.width,
                hdr.height,
...
pub(crate) fn load_detected_exr(
    ...
    let mmap = Arc::new(crate::mmap_util::map_file(path)?.0);
    load_detected_exr_from_mmap(path, mmap, cancel)
}
...
pub(crate) fn load_detected_exr_from_mmap(
    ...
    if let Some(image_data) =
        try_load_disk_backed_exr_hdr_from_mmap(path, Arc::clone(&mmap), cancel)?
    {
        return Ok(image_data);
    }

    // Full EXR display decode -- poll before chunk/tile orchestration.
    check_decode_cancel_str(cancel)?;
    let hdr = match crate::hdr::decode::decode_exr_display_image_from_mmap(
        ...
        Err(err) if is_exr_deep_data_unsupported_error(&err) => {
            ...
            return load_deep_exr_from_mmap(path, mmap, cancel);
        }
        Err(err) => return Err(err),
    };
    let fallback = DecodedImage::from_hdr_sdr_fallback(
