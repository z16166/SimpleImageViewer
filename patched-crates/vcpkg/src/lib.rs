...
impl Config {
    ...
    fn emit_libs(&mut self, lib: &mut Library, vcpkg_target: &VcpkgTarget) -> Result<(), Error> {
        for required_lib in &self.required_libs {
            ...
            // SimpleImageViewer: for Unix archive triplets (`*.a`), emit `static` so Cargo uses
            // `-Bstatic`/archive semantics. On Linux, add `-bundle` (i.e. `static:-bundle=`): the default
            // `+bundle` packs `.a` members into upstream rlibs so C++ objects are extracted late on the
            // final link; with traditional GNU bfd `ld` that can record DT_NEEDED `libstdc++.so.6` for
            // symbols like `std::condition_variable::notify_one@@GLIBCXX_*`. Release builds use `g++`
            // + workspace `.cargo/g++-lld-wrap.sh` as Cargo `linker` (unless `SIV_NO_LLD`) so bfd ld single-pass ordering is avoided.
            let link_line = if vcpkg_target.target_triplet.is_static
                && vcpkg_target.target_triplet.lib_suffix == "a"
            {
                let linux = env::var("CARGO_CFG_TARGET_OS").unwrap_or_default() == "linux";
                if linux {
                    format!("cargo:rustc-link-lib=static:-bundle={}", link_name)
                } else {
                    format!("cargo:rustc-link-lib=static={}", link_name)
                }
            } else {
                format!("cargo:rustc-link-lib={}", link_name)
            };
            lib.cargo_metadata.push(link_line);
